...
try
{
    ...
    builder.Services.AddCors(options =>
    {
        options.AddPolicy("CorsPolicy", policy =>
        {
            policy.WithOrigins(
                      "http://localhost:4200",
                      "https://diceus-claims-jmnxyait7hjje-frontend.azurewebsites.net",
                      "http://diceus-claims-jmnxyait7hjje-frontend.azurewebsites.net")
                  .AllowAnyHeader()
                  .AllowAnyMethod()
                  .AllowCredentials();
        });
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        try
        {
            ...
            // Migrate builds the database structure & seeds lookup and policies via migrations
